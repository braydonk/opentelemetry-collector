...
<!-- end autogenerated section -->`), 0600))
			...
			generatedPackageBasepath := filepath.Join(tmpdir, "internal", generatedPackageName)

			var contents []byte
			if tt.wantMetricsGenerated {
				require.FileExists(t, filepath.Join(generatedPackageBasepath, "generated_metrics.go"))
				require.FileExists(t, filepath.Join(generatedPackageBasepath, "generated_metrics_test.go"))
				require.FileExists(t, filepath.Join(tmpdir, "documentation.md"))
				contents, err = os.ReadFile(filepath.Join(generatedPackageBasepath, "generated_metrics.go")) // nolint: gosec
				require.NoError(t, err)
				...
				require.NoFileExists(t, filepath.Join(generatedPackageBasepath, "generated_metrics.go"))
				require.NoFileExists(t, filepath.Join(generatedPackageBasepath, "generated_metrics_test.go"))
			}

			if tt.wantConfigGenerated {
				require.FileExists(t, filepath.Join(generatedPackageBasepath, "generated_config.go"))
				require.FileExists(t, filepath.Join(generatedPackageBasepath, "generated_config_test.go"))
			} else {
				require.NoFileExists(t, filepath.Join(generatedPackageBasepath, "generated_config.go"))
				require.NoFileExists(t, filepath.Join(generatedPackageBasepath, "generated_config_test.go"))
			}

			if tt.wantTelemetryGenerated {
				require.FileExists(t, filepath.Join(generatedPackageBasepath, "generated_telemetry.go"))
				require.FileExists(t, filepath.Join(generatedPackageBasepath, "generated_telemetry_test.go"))
				require.FileExists(t, filepath.Join(tmpdir, "documentation.md"))
				contents, err = os.ReadFile(filepath.Join(generatedPackageBasepath, "generated_telemetry.go")) // nolint: gosec
				require.NoError(t, err)
				if tt.wantMetricsContext {
					require.Contains(t, string(contents), "\"context\"")
				} else {
				...
				require.NoFileExists(t, filepath.Join(generatedPackageBasepath, "generated_telemetry.go"))
				require.NoFileExists(t, filepath.Join(generatedPackageBasepath, "generated_telemetry_test.go"))
			}
			...
			if tt.wantStatusGenerated {
				require.FileExists(t, filepath.Join(generatedPackageBasepath, "generated_status.go"))
				contents, err = os.ReadFile(filepath.Join(tmpdir, "README.md")) // nolint: gosec
				require.NoError(t, err)
				require.NotContains(t, string(contents), "foo")
			} else {
				require.NoFileExists(t, filepath.Join(generatedPackageBasepath, "generated_status.go"))
				contents, err = os.ReadFile(filepath.Join(tmpdir, "README.md")) // nolint: gosec
				require.NoError(t, err)
