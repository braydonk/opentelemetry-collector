...
var flagPackageName = flag.String("package_name", "metadata", "The name of the package to generate")
...
func main() {
	...
	flag.Parse()
	yml := flag.Arg(0)
	fmt.Println(*flagPackageName)
	if err := run(yml, *flagPackageName); err != nil {
		log.Fatal(err)
	}
}

func run(ymlPath, generatedPackageName string) error {
	if ymlPath == "" {
